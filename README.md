...
| **Photo → solid (stub)** | Single PNG/JPG/WebP upload; silhouette + thickness + inferred backside cap (not a front-only relief); **repair-by-default** (fill cracks / interior holes) with a Keep damage / wear override; if the bbox exceeds the P2S 256³, the plate still exports and **designates** a stub alternate machine (K1 Max / H2D / Prusa XL) | Not photogrammetry or NeRF. Unseen geometry is a constant-thickness cap. WebP pixels are inferred from the file header (PNG/JPG decode the real silhouette). Not a machine farm or slicer picker |
...
5. **Image import as starting point** — **Foundations stub shipped:** user uploads a **single photo** (PNG/JPG/WebP); silhouette + thickness + inferred backside cap becomes a **full 3D printable solid** (not a front-only relief), repair-by-default, Keep damage / wear override, STL/3MF export, and oversize vs P2S designates a stub alternate machine. Later: photogrammetry / NeRF-quality unseen geometry (still in-app, no Blender).
