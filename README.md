...
- **Print doctor:** type a defect or machine complaint in the existing chat (`stringing with PETG`, `AMS 2 keeps looping feed/unfeed`), ask for a material table (`use PETG settings`, `best for PA`), or reassign a tray (`use AMS 2 for accent`). A keyword stub returns a diagnosis plus proposed settings or physical steps and can switch the selected material or update the AMS slot plan. After a reply, say **perfect** / **still bad** (or tap the tiny chips) to remember or reject that fix per printer + filament in the browser. It does not call the CAD pipeline and does not send surprise LAN commands.
...
6. Download **STL** and **3MF** (plus the `.scad` source and **Print settings** JSON), or one **project pack** zip (3MF + STL + stub build steps + shopping search terms + AMS slot plan). The selected Machine-panel material (PLA / PETG / PA / ABS / TPU) is written as advisory auto-best metadata on the 3MF and as `describeprint.print.json`. An **AMS slot plan** (tray 0–3 → material/color, live-preferring when connected) is written as `DescribePrint:ams_tray_*` metadata and `ams_slot_plan.json`. If the description names colors or materials, the 3MF also carries **separate objects** with `basematerials` display colors and extruder / AMS 1–4 metadata so a slicer can assign filaments. No colors mentioned → one default object. This is CAD export, not live AMS control. OpenSCAD itself is one mesh; split bodies need `region_*` modules or `color()` groups (the two-color fixture does this). Importing a colored 3MF preserves those objects.
