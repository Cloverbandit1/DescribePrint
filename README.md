...
- **Chat stays first-class.** The left Prepare column is a live describe/chat thread, not a dead slicer object list. Users keep talking to add, remove, or change the part; follow-ups send the previous description and OpenSCAD so the plate can update. On a phone-width viewport (~390px), Prepare is chat-first: a compact touch plate (orbit / pinch, or +/−) sits above the thread so the composer stays on-screen — chat is not hidden behind the plate. Preview keeps the plate large and still pins the composer. Chips, More options, and import actions use 44px hit targets. **Photo** opens the camera roll (`accept="image/*"`); **Camera** hints at the rear camera (`capture="environment"`). This is a responsive web client, not a native iOS app and not the Desktop Pack PWA host.
