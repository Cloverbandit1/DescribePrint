...
- The doctor bubble keeps Perfect / Still bad and adds a **Pause now** chip. That chip uses the existing mid-print `pause now` path (`sendMidPrintIfConnected`) — send only when already connected; otherwise “Connect LAN first”.
