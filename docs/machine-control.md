...
- When on, each `/api/machine` poll runs one stub `detectFailure()` (mock frames only). The panel shows `camera: ok · ok` or a **suspected** status line plus a severity cue (`Severity suspected — spaghetti. Pause recommended…`).
- A suspected failure opens **one** Print doctor chat with the matching camera guide (spaghetti / nozzle scrape / empty bed — AMS-style numbered steps). The same kind is not re-appended on later polls until the stub returns ok or the user dismisses (Perfect / Still bad). It does **not** send printer commands or auto-stop the job. `AMS_AUTOFIX` still only applies to AMS feed-loop.
- The doctor bubble keeps Perfect / Still bad and adds mid-print chips on that **one** announcement (not every poll): **Pause now** always; spaghetti also **Slow down** (speed tier 1) and **Cool nozzle -10°C** (clamped to the material/preset safe min); nozzle scrape may add **Slow down**; empty bed stays pause-only (no blind temp change). Chips use the existing mid-print phrase path (`sendMidPrintIfConnected`) — send only when already connected; otherwise “Connect LAN first”. Chat `slow down` / `cool nozzle` / absolute nozzle temps still work. Large temp jumps still pause first.
- `detectFailure()` returns `none` or `suspected-failure` with a print-doctor-style hint. Tests use the stub only — no pixels, no sockets. Guides live in [`lib/machine/camera-help.ts`](../lib/machine/camera-help.ts); debounce is [`lib/machine/camera-doctor-bridge.ts`](../lib/machine/camera-doctor-bridge.ts).

### Failure-photo replay (stub)

[`lib/machine/failure-photo.ts`](../lib/machine/failure-photo.ts) maps a **phone photo filename**, alt text, or a one-line hint onto the existing Print doctor path. It does **not** look at pixels and does not claim vision ML. A later classifier can replace the keyword table.

- Machine / Print panel: compact **Failed photo (stub)** file input + hint + Replay. Chat can also paste a short caption (`spaghetti blob`, `crash`).
- Mapping: spaghetti / string / blob → spaghetti; scrape / crash → scrape; empty / nothing → empty bed; ams / loop → AMS feed loop; `stringing` stays stringing. Unknown filename (or empty upload with no hint) **asks for a one-line hint** — no invented cause.
- A mapped result runs Print doctor (guides, Perfect / Still bad, pause / slow chips if connected). CAD repair is a **suggestion** only (`try reshape remaining` / `print doctor settings`). This stub does not call CAD fit-wizard, etch, lattice, or remaining-layer generate.
...
Chat can also say “use PETG settings” or “best for PA” to switch the Machine-panel material and apply that table. Those presets stay advisory (panel + export metadata). No CAD rewrite. No LLM. Chat-first: “stringing with PETG” or “AMS 2 keeps looping feed/unfeed”. A failed-print photo / caption uses the same doctor path via the filename-hint stub.
